...
# Introduction
In medicine, healthcare professionals are confronted daily with a wide range of information that needs to be processed in order to make clinical decisions. Due to the vast amount of information, prediction models, also known as prediction rules, prognostic models or risk scores, have been implemented with the intention to aid in decision making (1, 2). The way that prediction models help healthcare professionals is by informing about the risk of a specific outcome based on prediction variables (3). This type of decision making is closely associated with the movement towards stratified medicine, where decisions are based on each individual’s profile of prediction data (1).

The predictions that are made with a prediction model are generally based on multiple variables, and the outcome could either be a disease (diagnostic model) or an event that will occur in the future (prognostic model) (3, 4). In a diagnostic model, the predictions can be used to inform patients that their symptoms are not caused by a serious disease, to refer the patients to further testing and to initiate treatment (3). In a prognostic model, the predictions made can be used to choose between therapeutic options and to plan lifestyle changes within the period that has been observed (2, 5). This type of prediction could also be utilized to risk-stratify patients in therapeutic clinical trials (6, 7).

There are several well-known clinical prediction models that are daily being used within healthcare. The Ottawa Ankle Rules, CHA2DS2-VASc score and the Framingham Risk Score are just a few examples (8, 9, 10). To implement such models in healthcare, a few steps are required. The first step is to conduct a model development study (11). In these studies, a machine learning algorithm is used to develop a prediction model by using a dataset, defined as the development sample, that includes both predictor and outcome data from the development setting (11).  The machine learning algorithm searches for relationships in the data from the development sample, in order to calculate the risk of developing the specific outcome in a set of predictor data (12). The performance of the model is dependent on the algorithm and there is no typical rule of thumb for which algorithm one should choose (12). However, usually when the data in the development sample is limited, a simpler algorithm is utilized such as logistic regression (12). When the prediction model has been developed based on the algorithm, it usually tends to be optimistic when estimating performance within the development sample and needs to therefore be quantified for optimism by internal validation techniques (3).  The quantified optimism can thereafter be adjusted for by applying shrinkage or penalisation to the original model (13).

The second step is to conduct a validation study with or without model updating (14). During this step, the prediction model is tested in a transfer setting by utilizing a validation sample which contains new individuals that are similar but differ from those used to develop the model (14). This is done because developed and internally validated prediction models generally perform worse when they are introduced to a new set of individuals (14). If the performance of the model is poor within the validation sample, the model is of no use (15). However, adjustments could be made accordingly with the data collected from the validation sample, which is also preferable than to developing a new prediction model from scratch (14)

The final step is to perform an impact study (14, 15). During this step the impact of the prediction model is quantified and compared to not using the model, Ideally in a randomised trial, in variables such as behavioural changes in patients and healthcare professionals, patient health outcomes or cost-effectiveness of care (14, 15). If the impact of the prediction model is poor, it is of no use (15).

After completing all these steps, you can be more certain of whether the prediction model is applicable to the healthcare system from which you have developed the model. This however is easier said than done. As we previously mentioned in the validation study step, developed and internally validated prediction models generally perform poorly in the transfer setting (14). In order to even be able to obtain this information, one needs to collect both predictor and outcome data from the validation sample (14). This can be a problem that is both time inefficient and expensive if the data is difficult to access. In the case of the Framingham Risk Score, this problem is presented because of the outcome data only being accessible after 10 years of follow up (10).
...
Our hypothesis is that our method of estimating prediction model performance within a transfer setting by only utilizing predictor data, will be as good as current methods of calculating the performance within the transfer setting with available outcome data. This hypothesis is based on the fact that the segment created by the method is more similar to the validation sample and should therefore be able to constitute as such but with available outcome data.
...
# Methods
## Study design
We will conduct an analytical registry-based study. The dataset used for analysis has been made publicly available by the multinational, observational study by Eckert A et al, 2019 in the DRYAD repository (16, 21).
...
## Missing data
To account for missing data in our study we will carry out a complete case analysis because of only missing a few observations (17).

## Statistical methods  
### Statistical analysis  
Analysis in this study is all performed in the programming language python (19). Two-tailed tests are used and p-values of <0,05 are considered significant. To assess the difference in patient characteristics and model predictors between the participants being referred to the ICU and those that did not, we will use student’s t-test for numerical variables and chi-squared test for dichotomous variables. To assess the difference in accuracy between the new method that is developed and the current method of validating a prediction model with outcome data, we will use student’s t-test.
...
### Model development 
A prediction model is developed using the development sample in each dataset by using logistic regression implemented in the scikit-learn package in python (20). To avoid overfitting the model we will utilize bootstrapping to estimate a linear shrinkage factor that will be applied to the model coefficients (13). The shrunk model will then be used to calculate the accuracy of the model within the development sample.
...
The research data used in this study has been made freely reusable and citable in Dryad Digital Repository (21). Therefore the principle of autonomy is upheld due to there not being any requirement for informed consent.
...
[12][@deo2015machine] Deo RC. Machine learning in medicine. Circulation. 2015 Nov 17;132(20):1920-30.
[13][@steyerberg2001application]  Steyerberg EW, Eijkemans MJ, Habbema JD. Application of shrinkage techniques in logistic regression analysis: a case study. Statistica Neerlandica. 2001 Mar;55(1):76-88.  
[14][@moons2012risk]  Moons KG, Kengne AP, Grobbee DE, Royston P, Vergouwe Y, Altman DG, Woodward M. Risk prediction models: II. External validation, model updating, and impact assessment. Heart. 2012 May 1;98(9):691-8.  
...
**obs selfmade reference in cite.bib**  
[21] DRYAD [internet]. DRYAD; [cited 2020-09-23]. Available from: https://datadryad.org/stash/
