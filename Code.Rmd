...
# Discussion
## Key findings
At present, no methods exist that can predict the performance of a prediction model after it has been transferred while utilizing unlabeled data. In this study, we have developed and tested such a method. We found that our methods predicted accuracy were significantly worse than the apparent accuracy at predicting the accuracy in the country to which the prediction model was transferred. This was the case in the four of our six simulated transfers which suggest against our hypothesis, that our methods predicted accuracy would be as good or better than the apparent accuracy at predicting the accuracy of the prediction model after transfer.

## Naive and segmented approach
We also found that the accuracy in the transfer country was significantly worse than the accuracy in the country in which the model was developed. This was the case in five of our six simulated transfers, which suggest that our predictions were optimistic. These findings are consistent with the concluding remarks of Moons et al [@moons2012risk], that most prediction models perform worse in new sets of individuals that differ from the individuals that were used to develop the prediction models. Similar losses in performance has been reported in the prediction models that Ohnuma et al [@inserthere] reviewed.

We also found that the accuracy in the transfer countries were significantly worse than our methods predicted accuracy. This was the case in four of our six simulated transfers. This suggests that our predictions were still optimistic. These findings could be due to segmented samples being present in the sample that were used to train the prediction models. It could also be due to the outcomes being different although the model predictors being more similar. 
...
## Significance
Since we have found results that contradict our hypothesis, we recommend that our method should not be used to predict the accuracy of a prediction model after it has been transferred. This recommendation is based on this study, which has many limitations. Hence, more research is needed to provide more firm recommendations.
