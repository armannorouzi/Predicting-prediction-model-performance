...
#`r dftable(3)[[2]][1]` % (95% CI `r cis[1+12]` to `r cis[1+36+12]`) within `r strata.combinations[[3]][1]`, 
...
# Discussion
## Key findings
At present, no methods exist that can predict the performance of a prediction model after it has been transferred while utilizing unlabeled data. In this study, we have developed and tested such a method. We found that our method's predicted accuracy were significantly worse than the apparent accuracy at predicting the accuracy in the country to which the prediction model was transferred. This was the case in the four of our six simulated transfers which suggest against our hypothesis that our method's predicted accuracy would be as good or better than the apparent accuracy at predicting the accuracy of the prediction model after transfer.

## Naive and segmented approach
We found that the accuracy in the transfer country was significantly worse than the accuracy in the country in which the model was developed. This was the case in five of our six simulated transfers, which suggest that our initial predictions were optimistic. These findings are consistent with the concluding remarks of Moons et al [@moons2012risk], that most prediction models perform worse in new sets of individuals that differ from the individuals that were used to develop the prediction models. Similar losses in performance has been reported in the prediction models that Ohnuma et al [@inserthere] reviewed.

We also found that the accuracy in the transfer countries were significantly worse than our method's predicted accuracy. This was the case in four of our six simulated transfers. This suggests that our predictions were still optimistic. These findings could be due to segmented samples being present in the sample that were used to train the prediction models. It could also be due to the outcomes being different although the model predictors being more similar. 

## Other methods
Although this is the first method that predicts the performance of a prediction model after it has been transferred while utilizing unlabeled data, similar methods exist that predict the performance in similar populations as the data used to train the prediction model. Some of these methods include different types of bootstrapping, cross-validation and split-sampling [@insert4here]. Bootstrapping has been previously claimed to be the most efficient at predicting the performance in similar populations [@insert8here]. Steyerberg et al [@insert] showed that regular bootstrapping is to be preferred when they compared different methods that predict the performance in similar populations.

https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0169341#pone.0169341.ref035

https://www.tandfonline.com/doi/abs/10.1080/01621459.1983.10477973
https://books.google.se/books?hl=sv&lr=&id=gLlpIUxRntoC&oi=fnd&pg=PR14&ots=AauyS3KaF1&sig=Cw4WtZWZRspi_5QlJy-EuTMmvts&redir_esc=y#v=onepage&q&f=false
https://www.tandfonline.com/doi/abs/10.1080/01621459.1997.10474007
https://www.tandfonline.com/doi/abs/10.1080/00031305.1990.10475704

https://www-sciencedirect-com.proxy.kib.ki.se/science/article/pii/S0895435601003419
...
Our study has limitations. First, due to the design of our study being a secondary analysis of existing data, residual confounding is likely. Our results are therefore at best only hypothesis generating. To ideally test our method in future research, data should be collected for this specific hypothesis to minimize residual confounding<!-- I'm not convinced that this  is a major issue. I agree that it is an issue in studies of associations between variables but that's not what we're doing-->.

Second, due to small sample sizes in some of the included countries, we decided to simulate new samples. These new samples were likely poor representatives of real samples. Partly due to being simulated, but also due to the model outcomes being simulated using logistic regression models that were trained with the small sample sizes. This likely resulted in varying quality in the model outcomes in the different countries, which could have reflected badly on our method. To avoid this limitation in future studies, larger samples should be used and simulation of new samples should be avoided<!-- I don't think that the simulation in itself is bad, but I agree that using larger samples as the basis of simulation would be preferable-->.

Third, due to simplicity, we chose to only assess the performance of the prediction models in terms of the number of correct predictions compared to the total number of predictions (our accuracy). This was also done just for one specific decision threshold. To avoid these limitations in future studies, different types of measurements for performance should be studied such as the area under the receiver operating characteristic curve that is independent of the decision threshold. 

Fourth, due to us selecting model variables based on availability and them being continuous without any significance testing, it is likely that we produced selection bias and optimism in our prediction models as a result of overfitting [@insertmsh]. This may have reflected poorly on our method and should be avoided in future  research testing this method. 
https://www-bmj-com.proxy.kib.ki.se/content/338/bmj.b604.long

(side note)
Fifth, our method of selecting a prediction model that is less over-fitted by utilizing cross-validation is a method for reducing overfitting in models that use decision trees and not logistic regression. This also creates a problem where 80% of the development sample is used to train, while 100% is used to assess the accuracy which may lead to lower accuracy in the development sample. This however also affects our segmented sample as the segments might not always be present in the sample that is used to train the prediction model, which may lower the accuracy in the segmented sample. In future studies the prediction model should be trained on 100% of the development sample and its performance in that sample should be done with 100% of the development sample while overfitting should be reduced based on sample size and variable selection strategies.

Sixth, due to only assessing our method with the use of logistic regression, we can not draw conclusions on how our method would perform with other statistical algorithms. We argue however that this may not be a problem, as the difference should be minimal. 

## Significance
Since we have found results that contradict our hypothesis, we do not recommend that our method is used to predict the accuracy of a prediction model after it has been transferred. More research is needed to provide more firm recommendations.<!-- Because our method worked well in 2/6 simulations I think that there is scope to improve it. You may write something about this and why you think it worked well in those 2 instances -->

## Future studies
A study that is improving on the limitations of our study. Collecting data to reduce residual confounding, increasing the sample sizes to reduce overfitting and to avoid simulating new samples, measuring different types of performance independent of the decision threshold, reducing selection bias and optimism through model variable selection techniques.

A study that is comparing already established methods for predicting the performance of a prediction model within similar populations. These methods are used for predicting how the prediction models perform within similar populations and not after transfer. However, a comparison how these perform in contrast to our method could give further insight whether it is necessary to collect the unlabeled data.

# Conclusion
Even though our method is utilizing unlabeled data to predict the accuracy of a prediction model after it has been transferred, the results in this study suggest that our methods predictions are not as good or better than the accuracy that is acquired in the sample that is used to train the prediction model. 
