...
Another reason to why our method's predictions were still optimistic, could be due to the model outcomes varying despite having more similar model predictors. Which indicates that even if the model predictors are more similar, it does not necessarily mean that the participants received similar ICU admission.

## Transfers where our method was better / our method
Because most prediction models perform worse when they are introduced to new individuals, it would be beneficial for our method to predict an accuracy that is lower in most transfers when compared with the development sample accuracy. In the two transfer combinations where our method's predicted accuracy were significantly better than the development sample accuracy, this seemed to be the case. Although no significant difference was present between our method's predicted accuracy and the development sample accuracy in these two transfer combinations, the average of our method's predicted accuracy was lower.

One reason to why our method's predicted accuracy was lower on average in these two transfer combinations, could be due to a smaller proportion of the participants in the segmented samples being included in the training of the prediction models compared to the other transfer combinations. Another reason to why our method's predictions were lower on average could be that in these transfer combinations, the model outcomes in the segmented samples and the validation samples were more similar. 
...
**Borde jag lägga till något mer här?**
...
However, our study has its limitations. First, due to having small sample sizes in some of our countries, we decided to simulate new samples. These new samples were likely poor representatives of real samples. This is due to the model outcomes being simulated using logistic regression models that were trained with these small samples. This likely resulted in varying quality of the model outcomes in our different countries, which could have reflected poorly on our method. To avoid this limitation in future studies, larger sample should be used.
